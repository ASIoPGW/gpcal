...
\hline
$G$& Gravity constant&6.6742e-11&0.015 \%\\
...
$r_q$&Radius of quadrupole[m]&0.200 & 0.01 \%\\
$r_h$&Radius of hexapole[m]& 0.125 & 0.02\%\\
...
The gravity field calibrator modulate the mirrors gradient of gravity potential. However, its gravity gradient act the multi pendulum of the test mass. We estimated the responses of the each masses using transfer function of KAGRA as shown in Fig.~\ref{fig:ratio}. We can neglect the intermediate mass effect and regard as free mass motion larger than 30 Hz. We assumed the rotation frequency as 16 Hz, which is corresponding to 32 Hz and 48Hz at the operation frequency. of 2-f and 3-f components.  
\begin{figure}
\begin{center}
...
The uncertainty of the $V^R_{2f}$,$V^R_{3f}$, $V^R_{0}$ are the measured value. It implies that we can reduce the uncertainty with long time integration time due to the statistics. Each the uncertainty is listed in Table.~\ref{sus}. The estimated uncertainty of the displacement is 0.2 \%.
...
The estimated relative uncertainties of the powers are 0.4\%. One of the largest uncertainty is the geometrical factor of the Pcal laser. The geometrical factor uncertainty is assumed 0.3 \%, which is same number of LIGO. However, KAGRA employ the beam position monitoring system. 
