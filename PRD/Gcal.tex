...
To solve the uncertainty problem of the absolute calibration, we propose a new method by combining Pcal and Gcal. %a gravity field calibrator. (Gcal already defined)
 The Gcal generates the dynamic gravity field on %at the end of (??)
the test mass by rotating the multipole masses with a rotor placed in the vacuum chamber for isolating the acoustic noise. To monitor the frequency, we mount an %the 10-bit % do we need to mention 10-bit (less than 16-bit ??)
encoder with %its response using 
